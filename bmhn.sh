...
function generateChannelArtifacts() {
  ...
  echo "#################################################################"
  echo "### Generating channel configuration transaction 'channeltwo.tx' ###"
  echo "#################################################################"
  configtxgen -profile TwoOrgsChannelTwo -outputCreateChannelTx ./artifacts/channel/mychanneltwo.tx -channelID mychanneltwo
  if [ "$?" -ne 0 ]; then
    echo "Failed to generate channel configuration transaction..."
    exit 1
  fi
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile TwoOrgsChannelTwo -outputAnchorPeersUpdate \
    ./artifacts/channel/Org2MSPanchors.tx -channelID mychanneltwo -asOrg Org2MSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
